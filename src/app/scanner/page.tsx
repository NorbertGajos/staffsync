  ...
  async function handleSubstitution() {
    ...
    if (subReason === 'zastepstwo' && absentWorkerId) {
      ...
      if (absentShift) {
        ...
        if (absentAtt) {
          await supabase.from('attendance').update({ status: 'nieobecny' }).eq('id', absentAtt.id)
        } else {
          await supabase.from('attendance').insert({
          ...
          <div style={{ background:'white', borderRadius:'24px', padding:'28px', maxWidth:'420px', width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,0.4)' }}>
              ...
              {REASONS.map(r => (
                <button key={r.key} onClick={() => setSubReason(r.key as SubstitutionReason)}
                  style={{
                    ...
                    border: subReason === r.key ? '2px solid #0a6e8a' : '2px solid #ddeaf0',
                    background: subReason === r.key ? '#eef4fb' : 'white',
                    cursor:'pointer', textAlign:'left' as const, display:'flex', alignItems:'center', gap:'12px'
                  }}>
            ...
            {subReason === 'zastepstwo' && (
              <div style={{ marginBottom:'16px' }}>
                ...
                <select value={absentWorkerId} onChange={e => setAbsentWorkerId(e.target.value)}
            ...
            <div style={{ display:'flex', gap:'8px' }}>
              <button
                ...
                style={{
                  flex:1, padding:'14px',
                  background: (!subReason || (subReason === 'zastepstwo' && !absentWorkerId)) ? '#ccc' : '#0a6e8a',
                  color:'white', border:'none', borderRadius:'100px',
                  cursor: (!subReason || (subReason === 'zastepstwo' && !absentWorkerId)) ? 'not-allowed' : 'pointer',
                  fontSize:'14px', fontWeight:600
                }}>
                {savingSub ? 'Zapisuję...' : '✅ Potwierdź wejście'}
              </button>
              <button onClick={() => { setShowSubModal(false); processingRef.current = false; setProcessing(false) }}
